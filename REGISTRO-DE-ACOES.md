...
24. **Causa final da rejeição — sucesso.** A comparação confirmou que o Salgueiro aceita banco vazio estruturalmente válido na pré-atualização; o CELC exigia `lancamentos > 0` em toda validação. A regra foi separada: atualização aceita vazio, backup/restauração continuam estritos.
25. **Correção 0.2.15 — em preparação.** Inclui a aceitação segura de banco vazio na atualização e teste específico com SQLite real sem movimentações.
26. **Bootstrap local 0.2.15 — sucesso técnico, não prova online.** O `resources.neu` público e validado de 0.2.15 foi copiado para a instalação existente somente para permitir abrir o código corrigido; o pacote anterior foi preservado em `resources.neu.before-v0.2.15-bootstrap`. O Setup não foi executado e o ciclo pelo botão de atualização não foi testado nesta ação.
27. **Retirada do bootstrap — sucesso.** O cliente foi fechado e o `resources.neu` original foi restaurado a partir de `resources.neu.before-v0.2.15-bootstrap`; o hash voltou a `6602686dcb0927c4470071c7021e2de3296780186949d8fa6224b557d9e2035b`. Nenhuma atualização online foi declarada nesta ação.
