...
async function relatorios(){
  ...
  const cartaoCategoria=(item,tipo)=>`<article class="category-total-card ${tipo}"><div><span class="type-badge ${tipo}">${tipo}</span><b>${esc(item.categoria)}</b></div><small>${tipo==='entrada'?'Arrecadado no período':'Gasto no período'}</small><strong>${brl(item.realizado)}</strong><p>${item.quantidade} lançamento${item.quantidade===1?'':'s'} no período</p></article>`;
